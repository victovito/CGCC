/* Atividade do Módulo 6 - Victor Silva da Rosa */
...
const string windowName = "Módulo 6 - Victor Silva da Rosa";
const string controlPointsSource = "../src/control-points.txt";
const string vertexShaderSource = "../src/shaders/modulo6/vertex.shader.glsl";
const string fragmentShaderSource = "../src/shaders/modulo6/fragment.shader.glsl";
...
void updateObjects()
{
	for (int i = 0; i < objectsCount; i++) {
		...
		if (current.followPath) {
			int pointCount = current.controlPoints.size();
		
			current.pathPosition = fmod(current.pathPosition + deltaTime, pointCount);
			
			vec3 a = current.controlPoints[(int)floor(current.pathPosition)];
			vec3 b = current.controlPoints[(int)ceil(current.pathPosition) % pointCount];
			float relativePosition = fract(current.pathPosition);

			current.position = mix(a, b, relativePosition);
